...
WITH seating_types AS (
   SELECT DISTINCT
       ...
       CASE
           WHEN LOWER(CAST(approved_for_sidewalk_seating AS STRING)) IN ('yes', 'y', 'true', '1') THEN TRUE
           ELSE FALSE
       END AS approved_for_sidewalk,
       CASE
           WHEN LOWER(CAST(approved_for_roadway_seating AS STRING)) IN ('yes', 'y', 'true', '1') THEN TRUE
           ELSE FALSE
